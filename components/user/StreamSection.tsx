...
export default function StreamSection() {
  const streams = [
    { name: "Engineering / B.Tech", count: "12,402", icon: Monitor, color: "text-blue-600", bg: "bg-blue-50/50" },
    { name: "Management / MBA", count: "8,210", icon: Briefcase, color: "text-emerald-600", bg: "bg-emerald-50/50" },
    { name: "Medical / MBBS", count: "4,102", icon: Heart, color: "text-rose-600", bg: "bg-rose-50/50" },
    { name: "Design / B.Des", count: "1,200", icon: Palette, color: "text-purple-600", bg: "bg-purple-50/50" },
    { name: "Law / LLB", count: "840", icon: Scale, color: "text-amber-600", bg: "bg-amber-50/50" },
    { name: "Science / B.Sc", count: "3,100", icon: Microscope, color: "text-cyan-600", bg: "bg-cyan-50/50" },
    { name: "Aviation", count: "120", icon: Plane, color: "text-indigo-600", bg: "bg-indigo-50/50" },
    { name: "IT / B.CA", count: "240", icon: Gamepad2, color: "text-pink-600", bg: "bg-pink-50/50" },
  ];
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 space-y-8 md:space-y-0">
          <div className="space-y-4">
             <div className="flex items-center space-x-2 text-slate-400">
                ...
                <span className="text-[10px] font-medium uppercase tracking-[0.4em]">Browse by Category</span>
             </div>
             <h2 className="text-4xl md:text-6xl font-light text-slate-900 tracking-tighter leading-none">
                Explore Top <br className="hidden md:block" /> 
                <span className="italic font-medium text-slate-900/40">Degrees & Courses.</span>
             </h2>
          </div>
          <p className="text-slate-400 text-xs font-normal max-w-xs leading-relaxed uppercase tracking-widest text-right">
             Find detailed information on fees, admission, and placements for all streams.
          </p>
        </div>
          ...
          {streams.map((stream, idx) => (
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-slate-400 font-normal uppercase tracking-[0.2em]">{stream.count} Colleges</p>
              </div>

...
const universities: University[] = [
  { id: "U1", name: "University of Toronto", country: "Canada", city: "Toronto", qsRank: 21, acceptance: 43, tuitionUSD: 45000, ielts: 7.0, status: 'ACTIVE', partner: true },
  { id: "U2", name: "The University of Melbourne", country: "Australia", city: "Melbourne", qsRank: 14, acceptance: 70, tuitionUSD: 38000, ielts: 6.5, status: 'ACTIVE', partner: true },
  { id: "U3", name: "University College London", country: "UK", city: "London", qsRank: 9, acceptance: 63, tuitionUSD: 32000, ielts: 7.5, status: 'ACTIVE', partner: false },
  { id: "U4", name: "Purdue University", country: "USA", city: "West Lafayette", qsRank: 99, acceptance: 60, tuitionUSD: 28000, ielts: 6.5, status: 'PAUSED', partner: false },
];
  ...
  return (
    <div className="space-y-10 font-sans">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between space-y-6 md:space-y-0 pb-6 border-b border-gray-100">
        <div>
           ...
           <p className="text-secondary/40 text-sm font-bold uppercase tracking-widest mt-2">
              Managing 500+ High-Revenue International Partner Profiles
           </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
         ...
         {[
           { label: "Active Global Nodes", value: "542", trend: "+12.1%", icon: Building2, color: "text-primary bg-primary/5" },
           ...
           <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-gray-50 shadow-sm flex items-center justify-between group hover:border-primary/20 transition-all cursor-pointer">
              <div>
                 ...
                 <p className="text-4xl font-black text-typography tracking-tighter leading-none">{kpi.value}</p>
      ...
      <section className="bg-white rounded-[3rem] border border-gray-50 shadow-sm overflow-hidden flex flex-col font-sans">
                       ...
                       <td className="px-10 py-10">
                          <div className="flex items-center space-x-6 italic">
                             <div className={cn(
                               ...
                               uni.partner ? "bg-emerald-50 text-emerald-600 ring-1 ring-emerald-100" : "bg-snow-pearl text-secondary/10"
                             )}>
                                <Building2 size={28} />
                             </div>
                             <div>
                                <h4 className="text-base font-black text-typography leading-tight uppercase tracking-tight group-hover:text-primary transition-colors flex items-center space-x-2 italic">
                                   <span>{uni.name}</span>
                                   {uni.partner && <Zap size={10} className="text-emerald-500 fill-current" />}
                                </h4>
                                <div className="flex items-center space-x-3 text-[10px] font-bold text-secondary/20 uppercase tracking-widest mt-1 italic decoration-primary/10 underline">
                                   ...
                                   <span>{uni.partner ? 'Revenue Partner' : 'Listed Only'}</span>
                                </div>
                             </div>
                          </div>
                       </td>
                       <td className="px-10 py-10 text-center">
                          <p className="text-3xl font-black text-typography italic tracking-tighter">#{uni.qsRank}</p>
                          <p className="text-[8px] font-bold text-secondary/20 uppercase tracking-widest">Global Rank</p>
                       </td>
                       <td className="px-10 py-10 text-center">
                          <div className="inline-flex flex-col items-center">
                             <span className="text-xl font-black text-typography italic tracking-tighter">{uni.acceptance}%</span>
                             <div className="w-12 h-1 bg-snow-pearl rounded-full mt-1.5 overflow-hidden">
                                <div className="h-full bg-primary" style={{ width: `${uni.acceptance}%` }} />
                             </div>
                          </div>
                       </td>
                       <td className="px-10 py-10">
                          <div className="flex items-center space-x-2 text-[12px] font-black text-indigo-600 italic">
                             <BookOpen size={16} />
                             <span>{uni.ielts.toFixed(1)}</span>
                          </div>
                          <p className="text-[8px] font-bold text-secondary/20 uppercase tracking-widest mt-1">Band Req.</p>
                       </td>
                       <td className="px-10 py-10 text-right">
                          <p className="text-lg font-black text-typography italic tracking-tighter">{formatCurrency(uni.tuitionUSD)}</p>
                          <p className="text-[10px] font-bold text-secondary/30 uppercase tracking-widest leading-none mt-1 group-hover:text-primary transition-colors underline decoration-primary/10">${uni.tuitionUSD.toLocaleString()} USD</p>
                       </td>
                       <td className="px-10 py-10 text-right">
         ...
         <div className="p-10 border-t border-gray-50 flex items-center justify-between bg-white relative overflow-hidden italic font-sans">
            ...
            <div className="flex items-center space-x-6 relative z-10 italic">
               ...
               <p className="text-xs font-black text-secondary/40 uppercase tracking-widest italic decoration-primary/10 underline">Global Exchange Registry v2.1 • Currencies: USD → INR Polled via Fastify Service</p>
            </div>
            <div className="flex items-center space-x-4 relative z-10 font-sans not-italic">
               <button className="px-8 py-4 bg-snow-pearl rounded-2xl text-[10px] font-black uppercase tracking-widest text-secondary/40 hover:text-primary transition-all">Prev Page</button>
